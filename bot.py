...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text_lower in ["بن", "ban"]:
        ...
        await msg.reply_text(t(chat_id, "muted_temp", name=name, time=f"{minutes} دقیقه"))
        ...
        await msg.reply_text(f"✅ یک اخطار از {name} کم شد")
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cl.get("sticker") and msg.sticker:
        try: await msg.delete()
        except: pass
        return

    if cl.get("photo") and msg.photo:
        try: await msg.delete()
        except: pass
        return

    if cl.get("video") and msg.video:
        try: await msg.delete()
        except: pass
        return

    if cl.get("gif") and msg.animation:
        try: await msg.delete()
        except: pass
        return

    if cl.get("file") and msg.document:
        try: await msg.delete()
        except: pass
        return

    if cl.get("forward") and msg.forward_date:
        try: await msg.delete()
        except: pass
        return
