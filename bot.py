...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(t(chat_id, "start", name=user.first_name), reply_markup=main_menu_keyboard(chat_id))
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_main":
        ...
        txt = f"🎭 مدیریت استیکر:\n\nقفل استیکر: {status}" if l == "fa" else \
              f"🎭 Sticker Management:\n\nSticker lock: {status}"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cmds_fa = ["بن", "آنبن", "کیک", "سکوت", "آنسکوت", "ادمین", "عزل", "اخطار", "حذف‌اخطار", "اطلاعات", "اخطارها"]
    ...
    if text_lower.startswith("پاک") or text_lower.startswith("clean"):
        ...
        count = min(count, 100)
        try:
            deleted = await context.bot.delete_messages(chat_id, [msg.message_id])
    ...
    if text_lower in ["بن", "ban"]:
        ...
        try:
            ...
            status_map = {"creator": "مالک", "administrator": "ادمین", "member": "عضو", "restricted": "محدود", "left": "خارج شده", "banned": "بن شده"}
            status = status_map.get(member.status, member.status)
            l = get_lang(chat_id)
            if l == "fa":
                txt = f"👤 اطلاعات کاربر:\n\n🆔 آیدی: {target_id}\n📛 نام: {name}\n🏷 وضعیت: {status}\n⚠️ اخطار: {warn_count}/3"
            else:
